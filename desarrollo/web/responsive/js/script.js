...
$(document).ready(function($) {
	...
	try {
		...
		$(".owl-arrows .next-link").on('click', function(event){
			event.preventDefault();
			owl.trigger('owl.next');
		});
		$(".owl-arrows .prev-link").on('click', function(event){
			event.preventDefault();
	...
	try {
		...
		// Custom Navigation Events
		$(".owl-arrows2 .next-link").on('click', function(event){
			event.preventDefault();
			owl3.trigger('owl.next');
		});
		$(".owl-arrows2 .prev-link").on('click', function(event){
			event.preventDefault();
	...
	var contact = {"lat":"-34.441290", "lon":"-58.923612"}; //Change a map coordinate here!	
	...
	try {
		...
		mapContainer.gmap3({
			...
			marker:{
				options:{
					icon : new google.maps.MarkerImage('imagenes/logo_sm4.png'), 
					animation : google.maps.Animation.BOUNCE, position: {lat: -34.441290, lng: -58.923612},
					draggable: false,
	...
	var fieldNum = $('input[type="text"]'),
		...
		btnIncrease.on('click', function(event){
			...
			var nextVal = parseFloat(fieldVal, 10) + 1;
